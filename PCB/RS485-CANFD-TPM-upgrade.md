...
**Schematic symbols added**: `ADM2587E`, `ADM3055E`, and `SLB9672` schematic symbols (the `<symbol>` definitions only) have been added to the `Propio` library in `PCB/LibreServo-v2.3.1.sch`, pin-mapped from the datasheets in [`datasheets/`](datasheets/) (physically duplicated same-net pins, e.g. the four `GND1` pins on the ADM2587E, are collapsed onto a single symbol pin, matching this library's existing convention on parts like `MPM36XX`; true no-connect/not-connected-internally pins on the SLB9672 are likewise collapsed into single `NC`/`NCI`/`NCI_VDD`/`NCI_GND` pins rather than drawn 1:1 against silicon pin count).

**Still missing**: footprints/packages, devicesets (which pair a symbol with a footprint and map symbol pins to package pads), and 3D models for all three parts, plus the STM32G431 MCU symbol/footprint — none of those exist yet in the `Propio` library. Footprint pad geometry (pitch, pad size, courtyard, creepage/clearance for the two isolated SOIC-20 parts) is exactly the kind of thing that can look "done" in an XML diff while being wrong on the board, so that pass still needs doing in EAGLE itself with visual verification against the datasheet mechanical drawings, rather than hand-authored blind. Recommend doing the footprint/deviceset/placement work in EAGLE, using this document and the added symbols as the reference; happy to iterate on the design decisions further if useful.
