...
This document captures the part selection and the reasoning; it does **not** modify `PCB/LibreServo-v2.3.1.sch` or `.brd` beyond the schematic symbols noted below. Those are EAGLE 7.6 XML files.

**Schematic symbols added**: `ADM2587E`, `ADM3055E`, `SLB9672`, and `STM32G431` schematic symbols (the `<symbol>` definitions only) have been added to the `Propio` library in `PCB/LibreServo-v2.3.1.sch`, pin-mapped from the datasheets in [`datasheets/`](datasheets/) (physically duplicated same-net pins, e.g. the four `GND1` pins on the ADM2587E, are collapsed onto a single symbol pin, matching this library's existing convention on parts like `MPM36XX`; true no-connect/not-connected-internally pins on the SLB9672 are likewise collapsed into single `NC`/`NCI`/`NCI_VDD`/`NCI_GND` pins rather than drawn 1:1 against silicon pin count). The `STM32G431` symbol is UFQFPN32-only (pin numbers/functions extracted from `datasheets/stm32g431c6.pdf`'s Table 12 pin definition, `UFQFPN32` column) and, since K6-vs-K8 flash size is still an open item below and doesn't affect the pinout, is named at the family level rather than `STM32G431K6`/`STM32G431K8` — same pattern the existing `STM32F302` deviceset already uses relative to its `STM32F302K8` symbol.

**Still missing**: footprints/packages, devicesets (which pair a symbol with a footprint and map symbol pins to package pads), and 3D models for all four parts — none of those exist yet in the `Propio` library. Footprint pad geometry (pitch, pad size, courtyard, creepage/clearance for the two isolated SOIC-20 parts) is exactly the kind of thing that can look "done" in an XML diff while being wrong on the board, so that pass still needs doing in EAGLE itself with visual verification against the datasheet mechanical drawings, rather than hand-authored blind. Recommend doing the footprint/deviceset/placement work in EAGLE, using this document and the added symbols as the reference; happy to iterate on the design decisions further if useful.
